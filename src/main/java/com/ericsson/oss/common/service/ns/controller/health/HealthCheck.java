...
import lombok.Data;
/**
 * Health Check component for notification service.
 * Any internal logic can change health state of the notification service.
 */
@Data
...
public final class HealthCheck implements HealthIndicator {
    ...
    /**
     * The notification service specific management of the health endpoint (provided by default by the "actuator" component of 
     * Springboot).
     * @return The Health structure, which carries information about the health of a component or subsystem
     */
    @Override
    public Health health() {
        LOG.trace("Invoking Notification Service specific health check");
        final String errorCode = getErrorMessage();
        if (errorCode != null) {
            LOG.error("Health check is down with error : {}", errorCode);
